...
import { MicrosoftLogo, BloombergLogo, BoomiLogo, OhioStateLogo } from './CompanyLogos';
...
export default function About() {
  ...
  const getCertLogo = (issuer) => {
    ...
    if (issuer.toLowerCase().includes('boomi')) return <BoomiLogo className="w-4 h-4 shrink-0" />;
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {certifications.map((cert, cIdx) => (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="p-3.5 rounded-xl bg-slate-900/80 border border-slate-800 hover:border-blue-500/50 hover:bg-slate-850/90 transition-all flex flex-col justify-between group"
                    >
                      <div className="flex items-start justify-between gap-2 mb-2">
                        <div className="flex items-center gap-2.5 min-w-0">
                          {getCertLogo(cert.issuer)}
                          <div className="min-w-0">
                            <div className="text-xs font-semibold text-slate-200 group-hover:text-blue-400 transition-colors leading-snug">
                              {cert.name}
                            </div>
                      ...
                      {cert.skills && (
                        <div className="flex flex-wrap gap-1 mt-2 pt-2 border-t border-slate-800/60">
                          {cert.skills.map((skill, sIdx) => (
                            <span key={sIdx} className="text-[10px] px-2 py-0.5 rounded bg-slate-800 text-slate-400">
                              {skill}
                            </span>
                          ))}
                        </div>
